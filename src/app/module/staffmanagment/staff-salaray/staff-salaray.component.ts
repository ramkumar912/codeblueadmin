...
@Component({
  selector: 'app-staff-salaray',
  templateUrl: './staff-salaray.component.html',
  styleUrls: ['./staff-salaray.component.scss']
})

export class StaffSalarayComponent implements OnInit {
...
end_date:any='';
 staffsalarylist:any='';
  ...
  getServerData(event: number) {
    ...
    this. staffsalary_list(this.page_count)
  ...
  staffsalary_list(pagevalue: any) {
    this.loader = true;
    this.server.post("staffduty/salarylist?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.staffsalarylist = data.result.data
        this.total_item = data.result.total;
      } else if (data.status == 400) {
  ...
  go(pagevalue: any = 0) {
    this.loader = true;
    this.server.post("staffduty/salarylist?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this. staffsalarylist = data.result.data
  ...
  export() {
    this.loader = true;
    this.server.post("staffduty/exportsalary?orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      console.log(data)
    ...
    this.server.get("staffduty/salary/" + val).then((data: any) => {
      ...
      if (data.status == 200) {
        ...
        temp =data.result.data
          ...
          this.success_modal.view_staffsalary_modal(temp); 
