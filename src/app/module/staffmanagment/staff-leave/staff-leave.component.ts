...
end_date:any='';
 staffleavelist:any='';
  ...
  getServerData(event: number) {
    ...
    this. staffleave_list(this.page_count)
  ...
  staffleave_list(pagevalue: any) {
    this.loader = true;
    this.server.post("staffduty/leavelist?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.staffleavelist = data.result.data
        this.total_item = data.result.total;
      } else if (data.status == 400) {
  ...
  go(pagevalue: any = 0) {
    this.loader = true;
    this.server.post("staffduty/leavelist?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this. staffleavelist = data.result.data
  ...
  export() {
    this.loader = true;
    this.server.post("staffduty/exportleave?orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      console.log(data)
    ...
    this.server.get("staffduty/leave/" + val).then((data: any) => {
      ...
      if (data.status == 200) {
        ...
        temp =data.result.data
          ...
          this.success_modal.view_leave_modal(temp); 
