...
@Component({
  selector: 'app-staffexpense',
  templateUrl: './staffexpense.component.html',
  styleUrls: ['./staffexpense.component.scss']
})
export class StaffexpenseComponent implements OnInit {
  ...
  patient_view(val: any, check: any) {
    if (check == "view") {
      this.loader = true;
  ...
  getServerData(event: number) {
    ...
    this.expense_list(this.page_count)
  ...
  //==
  expense_list(pagevalue: any) {
    this.loader = true;
    this.server.post("staffexpense/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.expenselist = data.result.data
  ...
  go(pagevalue: any = 0) {
    this.loader = true;
    this.server.post("staffexpense/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.expenselist = data.result.data
  ...
  export() {
    this.loader = true;
    this.server.post("staffexpense/export?orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      console.log(data)
  ...
  expense_view(val: any, check: any) {
    if (check == "view") {
      this.loader = true;
      this.server.get("staffexpense/" + val).then((data: any) => {
        ...
        if (data.status == 200) {
          ...
          let temp = {};
          temp = data.result.data
          this.success_modal.view_expense_modal(temp);
          // this.patientlist = data.result.data
          // this.total_item = data.result.total;
        } else if (data.status == 400) {
          let result = [];
          data.result = [];
      ...
      this.router.navigate(['staff-expense-edit/' + val]);
    }
  }
