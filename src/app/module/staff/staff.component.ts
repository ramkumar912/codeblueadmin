...
export class StaffComponent implements OnInit {
  ...
  getServerData(event: number) {
    ...
    this.staff_list(this.page_count)
  ...
  staff_list(pagevalue: any): any {
    this.loader = true;
    this.server.post("staff/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
  ...
  staff_view(val: any, check: any) {
    if (check == "view") {
      this.router.navigate(['staff-view/' + val]);
    } else if (check == "edit") {
      this.router.navigate(['staff-edit/' + val]);
    }
  }

  go(pagevalue: any = 0) {
    this.loader = true;
    this.server.post("staff/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.stafflist = data.result.data
  ...
  export() {
    this.loader = true;
    this.server.post("staff/export?orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      console.log(data)
