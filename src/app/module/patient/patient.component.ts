...
export class PatientComponent implements OnInit {
 ...
 patient_view(val: any, check: any) {
  ...
  getServerData(event: number) {
    ...
    this.patient_list(this.page_count)
  ...
  patient_list(pagevalue:any){
    this.loader = true;
    this.server.post("patient/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.patientlist = data.result.data
  ...
  go(pagevalue: any = 0) {
    this.loader = true;
    this.server.post("patient/listdata?page=" + pagevalue + "&limit=10&orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      if (data.status == 200) {
        console.log(data.result.data)
        this.patientlist = data.result.data
  ...
  export() {
    this.loader = true;
    this.server.post("patient/export?orderby=id|desc", { search: this.search, fromdate: this.fromdateval, todate: this.todateval }).then((data: any) => {
      this.loader = false;
      console.log(data)
